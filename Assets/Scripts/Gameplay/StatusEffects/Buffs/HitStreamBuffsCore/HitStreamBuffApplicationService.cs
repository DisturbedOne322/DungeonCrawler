...
namespace Gameplay.StatusEffects.Buffs.HitStreamBuffsCore
{
    public class HitStreamBuffApplicationService
    {
        ...
        {
            _buffsByPriority = new SortedDictionary<int, List<HitStreamBuffInstance>>();
            _priorityCache = new Dictionary<HitStreamBuffPriorityType, int>();
            ...
            foreach (HitStreamBuffPriorityType priority in Enum.GetValues(typeof(HitStreamBuffPriorityType)))
            {
                ...
                _buffsByPriority[intValue] = new List<HitStreamBuffInstance>(DefaultBuffsCapacity);
